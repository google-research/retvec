...
class ScaledSinusoidalPositionalEmbedding(Layer):
    ...
    def __init__(self,
        ...
        self._scale = self.add_weight(name='sin_scale',
                                      shape=(),
    ...
    def get_config(self):
        ...
        base_config = super(
            ScaledSinusoidalPositionalEmbedding, self).get_config()
...
@tf.keras.utils.register_keras_serializable(package="retvec")
def rope(x: Tensor,
         axis: Union[List[int], int]) -> Tensor:
...
@tf.keras.utils.register_keras_serializable(package="retvec")
def toeplitz_matrix_rope(n: int,
                         a: Tensor,
                         b: Tensor) -> Tensor:
