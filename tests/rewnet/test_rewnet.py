...
from retvec.rewnet import REWCNN, REWBert, REWMix
from tensorflow_similarity.losses import MultiSimilarityLoss

tf.config.set_visible_devices([], 'GPU')

architectures = [REWCNN, REWBert, REWMix]
architectures_names = ['rewcnn', 'rewbert', 'rewmix']
...
def test_ori_decoder(NN):
    ...
    assert lyr.output.shape[2] == size
...
@pytest.mark.parametrize("NN", architectures, ids=architectures_names)
def test_complexity(NN):
    dim = 5
    model = NN(complexity_dim=dim)

    lyr = model.get_layer('complexity')
    assert lyr.output.shape[1] == dim
...
def test_lang(NN):
    dim = 5
    model = NN(lang_dim=dim)

    lyr = model.get_layer('lang')
    assert lyr.output.shape[1] == dim
